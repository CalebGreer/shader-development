Shader "Holistic/Section9/RaymarchCloudsVolume"
{
    Properties
    {
        ...
        SubShader
    {
        ...
        Pass
        {
            ...
            float noiseFromImage(float3 value)
            {
                value *= _Scale;
                value.x += _Time.x * 15;
            ...
            fixed4 integrate(fixed4 sum, float diffuse, float density, fixed4 bgcol, float t)
            {
                ...
                fixed3 colrgb = lerp(fixed3(1.0, 0.95, 0.9), fixed3(0.65, 0.65, 0.65), density);
